...
def analyze_bgp_paths(lab_name, router_name):
    ...
    total_prefixes = 0
    path_counts = {}  # prefix -> number of paths
...
def save_results_to_file(results, lab_name):
    with open(f"./scripts/out/bgp_path_diversity_results_{lab_name}.json", 'w') as f:
...
def compare_current_res_to_other_lab_res(lab_name, results, router_name):
    ...
    try:
        with open(f"./scripts/out/bgp_path_diversity_results_{other_lab}.json", 'r') as f:
